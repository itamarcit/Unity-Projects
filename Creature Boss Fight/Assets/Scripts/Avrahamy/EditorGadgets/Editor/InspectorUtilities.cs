...
namespace Avrahamy.EditorGadgets {
    public static class InspectorUtilities {
        ...
        /// </summary>
        public static bool ShowDropdownForStringProperty(
                Rect position,
                SerializedProperty property,
                IEnumerable<string> values,
                bool allowEmpty = true,
                bool condenseList = false) {
            ...
            foreach (var value in values) {
                var label = value;
                if (condenseList && !label.Contains('/')) {
                    label = "_/" + label;
                }
                menu.AddItem(
        ...
        public static FieldInfo GetFieldInfo(this SerializedProperty property) {
            ...
            if (property.depth > 0) {
                ...
                var type = instance.GetType();
                do {
                    fieldInfo = type.GetField(property.name, FLAGS);
                    type = type.BaseType;
                } while (fieldInfo == null && type != null);
            }
            return fieldInfo;
        ...
        public static Type GetFieldType(this SerializedProperty property) {
            object instance = property.serializedObject.targetObject;
            const BindingFlags FLAGS = BindingFlags.Default | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance;
            FieldInfo fieldInfo = null;
            if (property.depth > 0) {
                var elements = property.propertyPath.Split( '.' );
                for (var index = 0; index < elements.Length; index++) {
