...
namespace Pathfinding {
	...
	public interface ITraversalProvider {
		// (avrahamy) Added from node to implement JumpTraversal.
		bool CanTraverse (Path path, GraphNode from, GraphNode node);
		uint GetTraversalCost (Path path, GraphNode from, GraphNode node);
	}
	...
	public static class DefaultITraversalProvider {
		public static bool CanTraverse (Path path, GraphNode from, GraphNode node) {
			return node.Walkable && (path.enabledTags >> (int)node.Tag & 0x1) != 0;
		}

		public static uint GetTraversalCost (Path path, GraphNode from, GraphNode node) {
	...
	public abstract class Path : IPathInternals {
		...
		/// </summary>
		internal bool CanTraverse (GraphNode from, GraphNode node) {
			...
			if (traversalProvider != null)
				return traversalProvider.CanTraverse(this, from, node);

			unchecked { return node.Walkable && (enabledTags >> (int)node.Tag & 0x1) != 0; }
		}

		internal uint GetTraversalCost (GraphNode from, GraphNode node) {
#if ASTAR_NO_TRAVERSAL_COST
			return 0;
#else
			// Use traversal provider if set, otherwise fall back on default behaviour
			if (traversalProvider != null)
				return traversalProvider.GetTraversalCost(this, from, node);
