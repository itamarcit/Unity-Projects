...
namespace BitStrap
{
	public sealed class ShortcutsWindow : EditorWindow
	{
		...
		// (avrahamy) FIX: Selection was cleared when switching tabs.
		private bool focusJustChanged;
		...
		private void OnGUI()
		{
			...
			// Clear selection
			// (avrahamy) FIX: Added "eventType == EventType.MouseDown" so selection
			//                 will only be cleared on mouse down.
			if( clickCount > 0 && !clickedItem && eventType == EventType.MouseDown)
			{
